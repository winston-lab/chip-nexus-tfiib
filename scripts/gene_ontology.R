...
main = function(universe_path, diffexp_path, go_anno_path,
                factor_id, ttype, diffexp_direction,
    ...
    combined_plot = function(df, enrichment_direction){
        ggplot(data = df, aes(y=-log10(pval), x=term, fill=ontology)) +
            ...
            ggtitle(paste(enrichment_direction, "gene ontology terms"),
                    subtitle = paste(ttype, factor_id, "peaks", diffexp_direction, "in", condition, "vs.", control)) +
            theme_light() +
